...
7. **Example** (learning_progression only, when there is a script in `src/`): The **primary** example — one script that best demonstrates the pattern. Format: `**Example**: [script_name.py](../src/script_name.py)`. Use the actual filename that exists in the repo. If no script exists for the pattern, omit the **Example** line.
8. **Related** (learning_progression only, optional): Other `src/` scripts that use this pattern as a **secondary** technology (the pattern appears in them but is not the main focus). Format: `**Related**: [script_a.py](../src/script_a.py), [script_b.py](../src/script_b.py)`. Omit if there are no such scripts.
...
1. **Related patterns** (required for pattern docs)
   ...
   - Use relative links: `[link text](./filename.md)` for docs in this directory, `[link text](../src/script.py)` for example scripts.
...
2. **Learning path** (recommended)
   ...
   - Mention prerequisites (e.g. "Learn after Prompt Engineering and Structured Output") and, when applicable, link to an example script in `src/`.
...
- **Example links must point to existing files**: In learning_progression.md, **Example** (or **Example Files**) must reference only scripts that exist under `src/`. Verify filenames (e.g. `system_prompt_example.py`, `tool_use_patterns.py`, `schema_driven_translation.py`, `embeddings_vector_search.py`, `language_assessment_structured_output.py`, `understand_llm_models.py`). If a pattern has no example script, omit the Example line; use **Example Implementation** bullets only.
- **Example vs. Related**: **Example** is the single primary script that best demonstrates the pattern. **Related** lists any other `src/` scripts that use this pattern as a secondary technology (pattern appears but is not the main focus). When multiple scripts use the pattern, choose one as Example and list the rest under Related.
...
- **learning_progression**: Preferred order: Understanding Models → Prompts → Schemas → Tools → Schema-Driven Inference → Embeddings → tying together → production. Each pattern: Description, Technology, Key Concepts, Translation Example, Example Implementation; **Example** link only when script exists in `src/`.
