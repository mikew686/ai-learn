...
7. **Example** (learning_progression only, when there is a script in `src/scripts/examples/`): The **primary** example — one script that best demonstrates the pattern. Format: `**Example**: [script_name.py](../src/scripts/examples/script_name.py)`. Use the actual filename that exists in the repo. If no script exists for the pattern, omit the **Example** line.
8. **Related** (learning_progression only, optional): Other `src/scripts/examples/` scripts that use this pattern as a **secondary** technology (the pattern appears in them but is not the main focus). Format: `**Related**: [script_a.py](../src/scripts/examples/script_a.py), [script_b.py](../src/scripts/examples/script_b.py)`. Omit if there are no such scripts.
...
1. **Related patterns** (required for pattern docs)
   ...
   - Use relative links: `[link text](./filename.md)` for docs in this directory, `[link text](../src/scripts/examples/script.py)` for example scripts.
...
2. **Learning path** (recommended)
   ...
   - Mention prerequisites (e.g. "Learn after Prompt Engineering and Structured Output") and, when applicable, link to an example script in `src/scripts/examples/`.
...
- **From eng-dev-patterns to examples**: `../src/scripts/examples/embeddings_vector_search.py`, `../src/scripts/examples/schema_driven_translation.py`.
...
- Use markdown link form: `[link text](./path)` or `[link text](../src/scripts/examples/file.py)`.
...
- **Translation**: In learning_progression, translation is the shared use case; each pattern section should include a **Translation Example** and, when applicable, an **Example** link to a script in `src/scripts/examples/`.
...
- **Example links must point to existing files**: In learning_progression.md, **Example** (or **Example Files**) must reference only scripts that exist under `src/scripts/examples/`. Verify filenames (e.g. `system_prompt_example.py`, `tool_use_patterns.py`, `schema_driven_translation.py`, `embeddings_vector_search.py`, `language_assessment_structured_output.py`, `understand_llm_models.py`). If a pattern has no example script, omit the Example line; use **Example Implementation** bullets only.
- **Example vs. Related**: **Example** is the single primary script that best demonstrates the pattern. **Related** lists any other `src/scripts/examples/` scripts that use this pattern as a secondary technology (pattern appears but is not the main focus). When multiple scripts use the pattern, choose one as Example and list the rest under Related.
...
- **learning_progression**: Preferred order: Understanding Models → Prompts → Schemas → Tools → Schema-Driven Inference → Embeddings → tying together → production. Each pattern: Description, Technology, Key Concepts, Translation Example, Example Implementation; **Example** link only when script exists in `src/scripts/examples/`.
...
- Use **bold** labels, relative links (`./` and `../src/scripts/examples/`), and the same pattern-block structure so the documentation stays consistent and easy to navigate.
