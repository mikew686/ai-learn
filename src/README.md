...
Run from the project root with `python -m src.<module>`.
...
python -m src.understand_embedding_models [--model MODEL] [--embedding-model EMB] [--retrieve-after N] [--retrieve-k K]
```
...
```bash
python -m src.system_prompt_example [--model MODEL]
python -m src.language_assessment_structured_output [--model MODEL] [--prompt PHRASE] [--example-phrases]
python -m src.tool_use_patterns [--model MODEL] [--mode sequential|parallel|interleaved] [--target LANG] [--prompt PHRASE]
python -m src.schema_driven_translation [--model MODEL] [--prompt TEXT] [--target LANG]
python -m src.embeddings_vector_search [--model MODEL] [--embedding-model EMB] [--db PATH] [--top-k N]
```
