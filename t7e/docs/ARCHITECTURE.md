...
- **k8s (Kustomize)**: Base under `t7e/k8s/base/`: Deployment(s) for web and rqworker, Service(s), optional Ingress. **Local overlay only** deploys in-cluster Redis and Postgres (with pgvector); dev/prod overlays use **managed** Redis and Postgres (e.g. RDS, ElastiCache) and do not include those resources. Apply with `kubectl apply -k t7e/k8s/overlays/<env>`.
