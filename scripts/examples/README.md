...
Run from the project root with `python -m scripts.examples.<module>`.
...
python -m scripts.examples.understand_reasoning_model [--model MODEL] [--temperature T] [--max-tokens N]
python -m scripts.examples.understand_embedding_models [--model MODEL] [--embedding-model EMB] [--retrieve-after N] [--retrieve-k K]
```
...
```bash
python -m scripts.examples.system_prompt_example [--model MODEL]
python -m scripts.examples.language_assessment_structured_output [--model MODEL] [--prompt PHRASE] [--example-phrases]
python -m scripts.examples.tool_use_patterns [--model MODEL] [--mode sequential|parallel|interleaved] [--target LANG] [--prompt PHRASE]
python -m scripts.examples.schema_driven_translation [--model MODEL] [--prompt TEXT] [--target LANG]
python -m scripts.examples.embeddings_vector_search [--model MODEL] [--embedding-model EMB] [--db PATH] [--top-k N]
python -m scripts.examples.pg_vector_search [--model MODEL] [--embedding-model EMB] [--db-url URL] [--top-k N]
```
